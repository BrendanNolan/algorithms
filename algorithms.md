...
because your current node is the left child of its parent), return the current node.
...
including the root itself, must be black, so the black-heioght of the root is at least $h/2$, so
that $n \geq 2^{h/2} - 1$ and hence $n + 1 \geq 2^{h/2}$. Taking logs yields the result.

### RBT Operations (All Take $O(height) Time, Which Is $O(log n)$ Because RBTs Are Balanced)
...
$\sum_{i=1}^n = n(n+1)/2$, which is $O(n^2)$.
...
before $b$. Then, when $b$ is enqueued, we have $a.distance <= b.distance$.
...
only if, at the time that $u$ is discovered, there is a path $u\rightarrow v$ consisting entirely of
white vertices.
...
**Lemma:** If an edge crosses from SCC $C$ to SCC $C'$, then $f(C) > f(C')$.
...
of finishing times: $$C_n,C_{n-1},\ldots,C_0$$ Then by the above corollary, the above lineup is a
...
of $C_{n-1},\ldots,C_0$, so that indeed the first DFT hits exactly the vertices of $C_n$. The second
DFT similarly hits everything in $C_{n-1}$ and similarly hits none of $C_{n-2},\ldots,C_0$; it also
...
**Convergence Lemma:** If $s\rightarrow \ldots \rightarrow u \rightarrow v$ is a shortest path and
$u.d = \delta(s,u)$ at any time before $(u,v)$ is relaxed, then $v.d = \delta(s,v)$ at all times
afterwards.

The following is pretty easy to prove by induction on the length of the path: **Lemma:** If
$v_0,v_1,\ldots,v_n$ is a shortest path from $v_0$ to $v_n$ and the edges of this path are relaxed
in the order $(v_0,v_1),\ldots,(v_{n-1},v_n)$, possibly with other relaxations in between, then
$v_n.d = \delta(v_0,v_n)$ at all times after these relaxations.
